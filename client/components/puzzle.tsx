...
import GuessBox, { Guess } from 'components/guess_box';
...
export interface PuzzleData {
  ...
  solutionData?: SolutionData; // Only populated when solutions are visible
...
export interface PuzzleProps {
  ...
  localChecker?: string;
  children?: ReactNode | ((guesses: Guess[]) => ReactNode);
  solutionLinkOverride?: string;
  refreshOnSolve?: boolean;
}
...
const Puzzle: FC<PuzzleProps> = ({
  ...
  solutionLinkOverride = undefined,
  refreshOnSolve = false,
  children,
}) => {
  ...
  /* the team. */
  const websocketHandler = useCallback((message: Message = { data: {} }) => {
    if (message.data?.puzzle?.slug === puzzleData.slug) {
      ...
      if (message.data.guess) {
        // Freeze guess to hint to tsc that this is defined
        const frozenGuess = message.data.guess;
        setGuesses((guesses) => {
          // Make sure that we only update if the guess isn't already in the list.
          if (guesses.some((guess) => guess.guess === frozenGuess.guess)) {
            return guesses;
          }

          // Otherwise, prepend to the list.
          return [frozenGuess, ...guesses];
        });
        if (message.data.guess.isCorrect) {
          if (refreshOnSolve) {
  ...
  const getTopRightLinks = useMemo((): LinkData[] => {
    ...
    if (puzzleData?.solutionLinkVisible) {
      topRightLinks.push({
        href: solutionLinkOverride || `/solutions/${slug}`,
        text: 'View solution',
      });
      topRightLinks.push({
        href: `/stats/${slug}`,
  ...
  const displayedErrata = useMemo(
    () => (
      <div className="errata break-words">
        {errata.map((err, i) => (
          <p className="error" key={`errata-${i}`} suppressHydrationWarning>
            <b>Erratum</b> on{' '}
    ...
    >
        ...
        <div className="top-right flex flex-wrap justify-end space-x-2 print:hidden">
          {getTopRightLinks.map(({ href, text, shallow = false }, i) => (
            <Link
              ...
              href={href}
              shallow={shallow}
              className="bg-navbar p-1.5 mt-2 rounded-lg secondary"
            >
              {text}
            </Link>
          ))}
              ...
              <div className="guess-box flex flex-col justify-center text-center drop-shadow-md">
                ...
                >
                  ...
                  <div className="inputs flex relative justify-center w-full">
                    <Input
                      ref={inputRef}
                      className="font-mono grow shrink-0 basis-80 px-3 py-2 rounded-l rounded-r-none border-r-0"
                      name="answer"
                      id="answer"
                    ...
                    />
                    <input
                      className="ui-button font-smallcaps font-bold basis-24 rounded-l-none rounded-r"
                      id="submit-button"
                      type="submit"
        ...
        >
          ...
          {!isSolved &&
            ...
            interactions.map((interaction, i) => (
              <InteractionForm
                key={interaction.interactionSlug}
                {...interaction}
              />
            ))}
          {process.env.useWorker &&
            POSTHUNT_NEEDS_WORKER_PUZZLES.includes(slug) && (
              <WorkerLoadingIcon />
            )}
          {
      ...
      <style jsx>{`
        ...
        .container.maxWidth :global(section) {
          max-width: min(${maxWidthPx}px, 95vw);
        }
