...
// TODO: if monospace font changes, change here
const WEBSITE_MONOSPACE = '"DM Mono"';
// Font optimization does weird things
const WEBSITE_MONOSPACE_NEXTJS = '"__DM_Mono';
const DEFAULT_FONT_FAMILY = 'Arial, sans-serif';
// Roboto Mono for Sheets, Cascadia Mono for Excel.
// Main reason is visually separate underscores.
...
const CopyToClipboard: FunctionComponent<Props> = ({
  ...
  useEffect(() => {
    ...
    //
    // TODO: consider when to modify here, versus inserting copy-only elements
    // or using copy-only styles
    document.addEventListener('copy', (event) => {
      ...
      const cloned = recursiveClone(
        ...
        (node) => {
          ...
          // Don't lift styles into br
          if (node.tagName.toLowerCase() === 'br') return;

          // If we're looking at a hidden class, undo the hidden to lift
          // styles into inline properly
          let copyOnly = node.classList.contains(HIDDEN_CLASS);
          if (copyOnly) {
          ...
          // This applies only to nodes that are copied away.
          if (
            node.style.fontFamily.startsWith(WEBSITE_MONOSPACE) ||
            node.style.fontFamily.startsWith(WEBSITE_MONOSPACE_NEXTJS)
          ) {
          ...
          // Lift specific text decoration class from tailwind into shorthand class for Sheets
          if (node.style.textDecorationLine == 'underline') {
            node.style.textDecoration = 'underline';
          } else if (node.style.textDecorationLine == 'line-through') {
          ...
          if (node instanceof HTMLInputElement && node.dataset.copyInputValue) {
            // Note that this doesn't inherit styles directly on the input node
            return document.createTextNode(node.value);
          }

          // Google Sheets doesn't accept forms (and ignores their children), replace tag with div
          if (node.tagName.toLowerCase() === 'form') {
            const ret = document.createElement('div');
            ret.setAttribute('style', node.getAttribute('style') || '');
            return ret;
          }
        }
      );
  ...
  return (
    <>
      ...
      >
        <Twemoji
          emoji="📋"
          options={{ className: 'clip-twemoji' }}
          // Weird workaround for browsers that don't respect non-parent overflow
...
function copyJack(rootElement: HTMLElement, config: CopyConfig = {}) {
  ...
  // Lists don't work well with styles applied to specific items,
  // so convert them to tables
  for (const tag of ['ul', 'ol']) {
    for (const element of rootElement.querySelectorAll(
      tag + `:not(.${NO_COPY_CLASS})`
    )) {
      ...
      for (const li of element.querySelectorAll('li')) {
        ...
        // Nominally should copy other attrs... but that's a TODO
        tr.className = li.className;
  ...
  // If a crossword grid contains clues (#'s), make two copies.
  for (const crossword of rootElement.querySelectorAll(
    'table.crossword:not(.no-second-grid)'
  )) {
  ...
  for (const clue of rootElement.querySelectorAll('table.crossword .clue')) {
    const clueText = (clue as HTMLElement).innerText.trim();
    const hasOtherText =
      (clue.parentNode as HTMLElement).innerText.trim() !== clueText;
    const span = document.createElement('span');
    span.classList.add(HIDDEN_CLASS);
    span.innerText = !!clueText && hasOtherText ? '. ' : '';
...
function copyJackInlineStyles(
  ...
  // If we're looking at a copy-only class, undo hiding temporarily
  // Bug: this doesn't work if a parent is copy-only.
  let copyOnly = element.classList.contains(HIDDEN_CLASS);
  if (copyOnly) {
...
function maybeAppendResetSpan(element) {
  ...
  if (display.startsWith('inline')) {
    ...
    if (nonInlineParent.innerText.trim() === element.innerText.trim()) {
      // li's don't reset automatically
      if (nonInlineParent.tagName.toLowerCase() != 'li') {
        return;
      }
    }
  }
  ...
  // Assume that we want to reset to the parent element's style
  let resets = [
    'color',
    'font-family',
    'font-weight',
  ...
  for (let target of resets) {
    reset.style[target] = parentStyle[target];
  }
