...
const FreeAnswerForm: React.FC<Props> = ({ rounds, currency, used }) => {
  ...
  return (
    <Section title="Use Free Answer" narrow className="bg-white">
      <div className="mt-10 border border-dashed border-black p-2 rounded">
        <InfoIcon>
          You have{' '}
          <b>
            {currency} strong event reward{currency === 1 ? '' : 's'}
          </b>
          .
        </InfoIcon>
      </div>
          ...
          {userInfo?.public ? (
            ...
            <>
              ...
              <form onSubmit={onSubmit}>
                <select className="p-2 border" name="slug" id="slug">
...
export const getServerSideProps = async (context) => {
  ...
  if (process.env.isStatic) {
    ...
    props = await serverFetch<Props>(context, '/free_a3_answer', {
      method: 'GET',
    });
  }
  return { props };
