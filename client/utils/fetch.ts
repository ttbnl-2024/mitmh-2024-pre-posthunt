...
export const addDecryptionKeys = <T extends Object>({
  cryptKeys,
  ...rest
...
export const clientFetch = async <T>(
  ...
  // But, we cannot add SERVER_ENDPOINT for clientFetch because it triggers
  ...
  const basePath =
    'basePath' in context ? context.basePath : getBasePath(context);
...
export const useWebSocket = (
  ...
  const defaults = {
    ...
    reconnectInterval: (attemptNumber) =>
      Math.min((Math.pow(2, attemptNumber) - 1) * 1000, 20 * 1000),
    onError: (e) => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('beforeunload', () => {
      ...
      // If shared, it manages its own connections
      if (!options.share) {
...
export const getPuzzleSlug = () => {
  if (typeof window !== 'undefined') {
    ...
    if (match) {
      ...
      if (puzzle == "hydra-head") {
        const regexHydraPath = /\/puzzles\/hydra-head\/(\d+)/;
        const match = window.location.pathname.match(regexHydraPath);
        if (match) {
          return `hydra-head-${match[1]}`;
        } else {
          return null;
        }
      }
...
interface EventWebSocketProps {
  onJson?: (data: { key: string; data: any }) => any;
  key?: string;
