...
from puzzles.context import AuthedContextHttpRequest, ContextHttpRequest
...
from spoilr.core.api.decorators import ViewDecorator, ViewFunc
from spoilr.core.api.hunt import is_site_launched, is_site_solutions_published
from spoilr.core.models import User, UserAuth
...
def get_default_login_next_url(request: HttpRequest) -> str:
    site = get_site(request) or ""
            ...
            return JsonResponse(
                {
                    "form_errors": {
                        "__all__": (
                            "Cannot log in as an individual. Please use your team"
                            " login."
                        )
                    }
                },
...
def login_redirect(request: HttpRequest, parameters: dict[str, Any]) -> HttpResponse:
...
def get_login_redirect_url(request: HttpRequest, url: str) -> HttpResponse:
    """
...
def registration(request: ContextHttpRequest, slug: str) -> HttpResponse:
        ...
        return JsonResponse(
            {
                "form_errors": {
                    "__all__": (
                        f"You must be logged in to {request.method} registration info"
                        " for this team."
                    )
                }
            },
        ...
        return JsonResponse(
            {
                "form_errors": {
                    "__all__": (
                        f"You do not have permission to {request.method} registration"
                        " info for this team."
                    )
                }
            },
        ...
        return JsonResponse(
            {
                "form_errors": {
                    "__all__": (
                        "You do not have any registration info. Please contact us if"
                        " you are not an internal team."
                    )
                }
            },
            status=401,
    ...
    if request.method == "POST":
            ...
            return JsonResponse(
                {
                    "form_errors": {
                        "__all__": (
                            "Hunt has already started, and registration is closed."
                        )
                    }
                },
                status=400,
        ...
        team_edit_form = TeamEditForm(
            request.POST, request.FILES, instance=team.teamregistrationinfo
        )
        ...
        team_reg, _ = TeamRegistrationInfo.objects.update_or_create(
            team_id=team.id, defaults=team_edit_form.cleaned_data
        )
        team_reg.save()

        return JsonResponse(
            {"data": {"constellation_pic_url": team_reg.constellation_pic.url}}
        )
        ...
        return JsonResponse(
            {
                ...
                # ImageField points to the URL
                "constellation_pic_url": (
                    registration_info.constellation_pic.url
                    if registration_info.constellation_pic
                    else ""
                ),
                **model_to_dict(
                    registration_info,
...
def register(request: HttpRequest) -> HttpResponse:
    ...
    team_form = TeamCreationForm(request.POST, request.FILES)
...
@require_http_methods(["GET", "POST"])
def register_individual(request: ContextHttpRequest) -> HttpResponse:
            ...
            return JsonResponse(
                {
                    "form_errors": {
                        "__all__": (
                            "You must be logged in to get registration info for this"
                            " user."
                        )
                    }
                },
    ...
    if request.method == "POST":
        ...
        if not request.user.is_authenticated:
            ...
            with transaction.atomic():
                user = user_form.save(commit=True)
                individual_form.save(user)

            login(request, user)
            return JsonResponse({})
        else:
            authed_request = cast(AuthedContextHttpRequest, request)
            individual_edit_form = IndividualEditForm(
                request.POST, instance=authed_request.user.individualregistrationinfo
            )
...
def perform_validate_puzzle(
    ...
    if (
        ...
        return JsonResponse(
            {
                "error": (
                    "You must be signed in and have a registered team to access this"
                    " page."
                ),
            },
            status=404,
...
def validate_puzzle(
    require_team: bool = False, allow_after_hunt: bool = False
) -> ViewDecorator:
    """
    ...
    def decorator(f: ViewFunc) -> ViewFunc:
        @wraps(f)
        def inner(
            request: ContextHttpRequest, slug: str, *args: Any, **kwargs: Any
        ) -> HttpResponse:
...
def restrict_access(after_hunt_end: bool | None = None) -> ViewDecorator:
    """
    Indicates an endpoint that is hidden to all regular users. Superusers and
    staff are always allowed access. Behavior depends on after_hunt_end:
    ...
    def decorator(f: ViewFunc) -> ViewFunc:
        @wraps(f)
        def inner(
            request: ContextHttpRequest, *args: Any, **kwargs: Any
        ) -> HttpResponse:
            is_internal = request.path.lstrip("/").split("/")[0] == "internal"
            if not request.context.is_superuser and not request.context.is_staff:
...
def solution_allowed(request: ContextHttpRequest) -> bool:
    return (
...
def check_access_allowed(
    request: ContextHttpRequest, original_path: str
) -> HttpResponse:
    """
    All gating is done by url paths, not parameters, but the full url is
    returned if valid.
    """
    parts: dict[int, str | None] = defaultdict(
        lambda: None, enumerate(original_path.split("/"))
    )
    ...
    if resource_type in ("puzzle", "hint"):
        if slug:
            if slug == "hydra-head":
                slug = f"hydra-head-{subpart}"
                subpart = ""
            allowed, _ = request.context.is_unlocked(slug)
        if allowed and resource_type == "puzzle" and slug and subpart:
        ...
        allowed = request.context.hunt_is_over or bool(
        ...
        allowed = request.context.is_superuser or request.context.is_staff
