...
class StoryCard(SlugModel):
    ...
    # TODO: stuff is missing here (for unlocking), not sure what
    ...
    # Hack to allow arbitrary resources to be served
    @property
    def audio_url(self) -> str | None:
        return get_hashed_url(f"Story/{self.slug}.mp3")
    ...
    def url(self):
        ...
        return generate_url("hunt", "oracle")
...
class StoryCardAccess(models.Model):
    ...
    def __str__(self) -> str:
        return f"{self.team} -> {self.story_card} @ {self.timestamp}"
...
class StoryState(models.Model):
    ...
    @classmethod
    def singleton(cls, team: Team | int) -> "StoryState":
