...
class ImapClient:
    ...
    def __init__(
        ...
        self.server: imapclient.IMAPClient | None = None
    ...
    def connect(self):
        ...
        self.close()
        try:
            self.server = imapclient.IMAPClient(self.host, self.port)
        ...
        while True:
            try:
                ...
                if last_error_timestamp is not None:
                    ...
                    django_logger.info("IMAPClient connected!")
                last_error_timestamp = None
                while True:
                    self.fetch()
                    self.server.idle()
                    try:
                        self.server.idle_check(timeout=self.timeout)
                    finally:
    ...
    def process(self, uid, date, modseq, raw_content):
        ...
        if root_reference_id is not None:
            ...
            except Email.hint.RelatedObjectDoesNotExist:  # type: ignore
                ancestor_hint = None
            ...
            if ancestor_hint:
                Hint.objects.get_or_create(
                    ...
                    defaults={
                        ...
                        "notify_emails": (
                            ", ".join(them_addresses) if them_addresses else "none"
                        ),
                    },
                )
    ...
    def create_and_run(cls):
        ...
        with cls(
            ...
            account=settings.EMAIL_IMAP_USER,
    ...
    *,
    ...
    with redis_lock(
        ...
        if invalid_recipients:
            task_logger.warn(
                f"Got ill-formatted recipients {invalid_recipients} for email with"
                f" Subject: {email_message.get('Subject', '')} Message-Id:"
                f" {email_message.get('Message-ID')}. Ignoring."
            )
        if len(recipients) != len(all_recipients):
            task_logger.info(
                "Would have sent email with Subject:"
                f" {email_message.get('Subject', '')} Message-Id:"
                f" {email_message.get('Message-ID')} to {all_recipients}. Instead"
                f" sending to {recipients}."
            )
        ...
        if recipients:
            task_logger.info(
                ...
                f" {email_message.get('Subject', '')!r} Message-Id:"
                f" {email_message.get('Message-ID')} to {recipients}."
            )
...
def _get_email_template_batches(template_obj):
    ...
    if template_obj.recipients == EmailTemplate.RECIPIENT_TEAMS:
        ...
        batched_addresses: list[tuple[int, str]] = [
            (idx, address)
        ...
        for batch_iter in itertools.zip_longest(
            *(iter(batched_addresses),) * batch_size
        ):
            batch: list[tuple[int, str]] = list(filter(None, batch_iter))
            batches.append(
                Batch(
                    user=None,
                    team=None,
                ...
                with transaction.atomic():
                    ...
                    if email_obj.all_recipients:
                        ...
                        transaction.on_commit(
                            lambda eo=email_obj: task_send_email(  # type: ignore
                                eo,
                                active_connection=connection,
                            )
                        )
...
def email_obj_for_batch(email_template, batch, message_id=None):
    ...
    context = {
        "because_registered": (
            email_template.recipients != EmailTemplate.RECIPIENT_BATCH_ADDRESSES
        ),
    }
    if batch.team is not None:
