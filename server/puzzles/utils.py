...
def login_required(function) -> ViewFunc:
...
def is_unlocked(*, puzzle_slug=None, story_slug=None, user=None, team_id=None):
    ...
    if not team:
        return False
    if puzzle_slug and team.puzzleaccess_set.first():  # type: ignore
        return True
    if story_slug and team.storycardaccess_set.first():  # type: ignore
        return True
...
def get_encryption_keys(names: list[str]):
...
def get_redis_handle():
    return redis.Redis(
        ...
        db=settings.REDIS_DATABASE_ENUM.REDIS_CLIENT.value,  # type: ignore
    )
...
def hint_availability(
    request: AuthedContextHttpRequestWithPuzzle, puzzle: Puzzle, team
) -> tuple[HintVisibility, str]:
    if not team:
    ...
    puzzle_access = PuzzleAccess.objects.filter(team=team, puzzle=puzzle).first()
    timed_hint_rounds = {
        "las-vegas",
        "everglades",
        "mississippi-river",
    ...
    if not (
        ...
        or (
            ...
            and puzzle.round.slug in timed_hint_rounds
            and request.context.now - puzzle_access.timestamp >= timedelta(hours=6)
        )
    ):
    ...
    if num_open >= get_max_open_hints():
        ...
        return (
            HintVisibility.CAN_VIEW,
            (
                f"You have {num_open} outstanding {hints}. Please wait for us to get"
                " back to you before requesting more hints."
            ),
        )
