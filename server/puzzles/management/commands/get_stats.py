...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        teams = Team.objects.annotate(
            num_solves=Count(
                "puzzlesubmission", filter=Q(puzzlesubmission__status="correct")
            )
        )

        def with_solves(n):
            return teams.filter(num_solves__gte=n).count()
        ...
        teams = Team.objects.annotate(
            num_solves=Count(
                "puzzlesubmission",
                filter=Q(
                    puzzlesubmission__status="correct",
        ...
        solves = PuzzleSubmission.objects.filter(status="correct")
