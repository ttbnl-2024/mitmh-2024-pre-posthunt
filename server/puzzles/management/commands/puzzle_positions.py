...
class Command(BaseCommand):
    ...
    def _puzzle_path(self) -> str:
        return os.path.join(self.puzzle_directory, "positions.csv")

    def _save_puzzle_positions(self) -> None:
        data = []
    ...
    def _load_puzzle_positions(self) -> None:
        path = self._puzzle_path()
        ...
        try:
            with open(path) as f:
                ...
                for row in reader:
                    ...
                    if row["slug"] not in puzzles_by_key:
                        self.stdout.write(
                            f"Unable to find puzzle with slug {row['slug']}. Please"
            ...
            msg = f"Unable to read file {path}. Aborting."
    ...
    def handle(self, *args, **options) -> None:
        ...
        def check_dir(path):
            if not os.path.isdir(path):
                msg = f"Invalid path specified: {path}"
