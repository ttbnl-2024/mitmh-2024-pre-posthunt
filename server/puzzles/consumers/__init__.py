...
class ClientConsumer(AsyncJsonWebsocketConsumer):
    ...
    @staticmethod
    def get_team_group(user=None, *, id: int | None = None, suffix: str | None = None):
        if id is None:
            if user is None:
                msg = "must provide user or id"
    ...
    def get_session_group(user=None, session_id=None, *, team_id=None):
        ...
        if team_id is None:
            if user is None:
                msg = "must provide user or team_id"
                raise ValueError(msg)
    ...
    def get_handler(self, puzzle_slug) -> type[BasePuzzleHandler] | None:
        from puzzles.consumers.puzzles import gaia, junky_logic, make_a_winning_hand

        SLUG_HANDLERS: Mapping[str, type[BasePuzzleHandler]] = {
            # FIXME: Add handlers for puzzles and story
            "junky-logic": junky_logic.JunkyLogicHandler,
            "gaia": gaia.GaiaHandler,
            "make-a-winning-hand": make_a_winning_hand.WinningHandHandler,
        }
    ...
    @staticmethod
    def send_event(group, key, data, crypt_keys: dict[str, str] | None = None):
        """
        ...
        cryptKeys: A dict of crypt keys
        """
        crypt_keys = crypt_keys or {}
        channels_data = {
            "type": "handle.event",
            "event": {
                "key": key,
                "data": data,
                "cryptKeys": crypt_keys,
            },
        }
