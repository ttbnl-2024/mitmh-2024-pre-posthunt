...
def add_uuid(lst: list[str], uuid: str) -> list[str]:
    return list(set(lst) | {uuid})


def remove_uuid(lst: list[str], uuid: str) -> list[str]:
    return [v for v in lst if v != uuid]
...
def complete_session(slug, session, team):
    SLUG_TO_STORY_STATE: dict[str, StateEnum] = {
...
class PuzzleHandler(BasePuzzleHandler):
    ...
    def serialize_data(session: dict):
        ...
        session["state"].get("selected_text")
    ...
    def process_data(user, uuid, data, puzzle_slug=None, **kwargs):
        ...
        with story_session_cache_manager(user.team_id, storycard_id) as cm:
            ...
            if session is None:
                ...
                msg = f"No story found for slug {puzzle_slug}"
    ...
    def disconnect(user, uuid, slug, **kwargs):
        ...
        with story_session_cache_manager(user.team_id, storycard_id) as cm:
            ...
            if session is None:
                # should not happen unless we're mucking in admin
                msg = f"No story found for slug {slug}"
                raise RuntimeError(msg)
