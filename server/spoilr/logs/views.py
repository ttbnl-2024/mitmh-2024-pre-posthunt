...
from spoilr.core.models import Puzzle, SystemLog, Team
...
def system_log_csv_export(request):
    # Filter out system log events that we shouldn't publicize, i.e., email responses.
    ...
    fname = "tph_guesslog_{}.csv".format(timezone.now().strftime("%Y%m%dT%H%M%S%z"))
...
@hq()
def hint_log_view(request, limit: int = 1000):
    entries = [
        {
            "entry": entry,
            "task": entry.tasks.first(),
        }
        for entry in Hint.objects.select_related("team", "puzzle")
