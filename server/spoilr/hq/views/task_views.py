...
from puzzles.messaging import dispatch_bot_alert
from spoilr.hq.models import HqLog, Task, TaskStatus
from spoilr.hq.util.decorators import HqHandlerHttpRequest, hq
from spoilr.hq.util.redirect import redirect_with_message

DispatchableContent: TypeAlias = (
    spoilr.email.models.Email
    | spoilr.hints.models.Hint
    | spoilr.interaction.models.TeamInteraction
)


@require_POST
@hq(require_handler=True)
def task_claim_view(request: HqHandlerHttpRequest):
    ...
    if not force_reopen and any(
        task.status in (TaskStatus.DONE, TaskStatus.IGNORED) for task in tasks
    ):
        return redirect_with_message(
            request,
            "spoilr.hq:dashboard",
    ...
    for task in tasks:
        ...
        if yoink and old_handler:
            logs.append(
                HqLog(
                    ...
                    object_id=task.id,
                    message=(
                        f"Yoinked task {task.content_object} from {old_handler.discord}"
                    ),
                )
            )
        else:
        ...
        if task.content_type.model_class() in (
            spoilr.email.models.Email,
            spoilr.hints.models.Hint,
            spoilr.interaction.models.TeamInteractionTask,
        ):
            obj = task.content_object  # type: ignore
            if (
                task.content_type.model_class()
                == spoilr.interaction.models.TeamInteractionTask
            ):
                obj = cast(
                    spoilr.interaction.models.TeamInteractionTask, obj
                ).team_interaction
            obj = cast(DispatchableContent, obj)
...
@require_POST
@hq(require_handler=True)
def task_unclaim_view(request: HqHandlerHttpRequest):
    task_ids = request.POST.getlist("task_id")
    tasks = Task.objects.filter(id__in=task_ids)
    if not len(tasks):
        return redirect_with_message(
    ...
    if any(task.handler != request.handler for task in tasks):
        return redirect_with_message(
            request, "spoilr.hq:dashboard", "You are no longer handling the task(s)."
        )
    ...
    for task in tasks:
        ...
        if task.content_type.model_class() in (
            ...
            if (
                ...
                obj = task.content_object.team_interaction  # type: ignore
            obj = cast(DispatchableContent, obj)
...
@require_POST
@hq(require_handler=True)
def task_snooze_view(request: HqHandlerHttpRequest):
    task_ids = request.POST.getlist("task_id")
    tasks = Task.objects.filter(id__in=task_ids)
    snooze_hours = float(request.POST.get("snooze_hours") or "0")
    if not len(tasks):
        return redirect_with_message(
            request, "spoilr.hq:dashboard", "No tasks selected."
        )
    ...
    for task in tasks:
        ...
        logs.append(
            HqLog(
                ...
                object_id=task.id,
                message=(
                    f"Snoozed task {task.content_object} for {snooze_hours:.2f} hour(s)"
                ),
            )
        )
...
@require_POST
@hq(require_handler=True)
def task_ignore_view(request: HqHandlerHttpRequest):
    confirm = request.POST.get("confirm") or ""
    ...
    if not task.handler or task.handler != request.handler:
        return redirect_with_message(
            request, "spoilr.hq:dashboard", "You are no longer handling this task."
        )
