...
def closest_hour(timestamp: datetime.datetime, start: datetime.datetime) -> int:
...
def solve_graph_view(request):
    ...
    curr = start
    while curr < end:
        hour_labels.append(int(curr.timestamp() * 1000))
    ...
    team_sizes_by_id: dict[int, int] = {
        team.id: team.teamregistrationinfo.tm_total  # type: ignore
        for team in Team.objects.exclude(type=TeamType.INTERNAL)
    ...
    solve_counts_by_team: dict[int, int] = collections.defaultdict(int)
    solve_counts_time_series_by_team: dict[int, list[tuple[int, int, int]]] = (
        collections.defaultdict(list)
    )
    point_styles_by_team: dict[int, list[str]] = collections.defaultdict(list)
    ...
    all_puzzle_subs = (
        ...
        .exclude(puzzle__tph_puzzle__hydra_puzzle__isnull=False)
        .filter(
    ...
    correct_puzzle_subs = all_puzzle_subs.filter(status="correct").values_list(
        "team_id",
        "puzzle_id",
        "puzzle__slug",
        "timestamp",
    )
    all_correct_subs: list[tuple[int, int, str, datetime.datetime]] = list(
        correct_puzzle_subs
    )
    all_correct_subs.sort(key=lambda info: info[3])  # solve-time

    free_answers_by_hour: dict[int, int] = collections.defaultdict(int)

    style_map = get_style_map()

    for team_id, puzzle_id, puzzle_slug, solved_time in all_correct_subs:
        solve_counts_by_team[team_id] += 1
        solved_time_as_unix = int(solved_time.timestamp() * 1000)
    ...
    # Hint counts by hour
    hint_requests_by_hour: dict[int, int] = collections.defaultdict(int)
    hint_responses_by_hour: dict[int, int] = collections.defaultdict(int)
    contact_hq_requests_by_hour: dict[int, int] = collections.defaultdict(int)
    contact_hq_responses_by_hour: dict[int, int] = collections.defaultdict(int)
    ...
    team_size_ids = list(team_sizes_by_id.keys() & solve_counts_by_team.keys())
    guess_count_ids = list(guess_counts_by_id.keys() & team_size_ids)
    ...
    return render(
        ...
        "spoilr/progress/solves.tmpl",
        {
            ...
            "solve_counts_for_chartjs": {
                ...
                "pointLabels": [
                    [
                        f"{team_names_by_id[team_id]} solved"
                        f" {puzzle_names_by_id[solve_count[2]]}, "
...
def solve_data_json_view(request):
    ...
    puzzle_accesses: list[tuple[int, int, datetime.datetime]] = list(
        # https://github.com/typeddjango/django-stubs/issues/922
        PuzzleAccess.objects.exclude(team__type=TeamType.INTERNAL)  # type: ignore
