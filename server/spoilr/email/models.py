...
class EmailTemplate(models.Model):
    ...
    RECIPIENT_OPTIONS = {
        RECIPIENT_TEAMS: "all_teams",  # send to teams individually (teammembers too)
        RECIPIENT_BATCH_ADDRESSES: (
            "batch_addresses"
        ),  # send to batches of addresses (bcc)
    }
...
class CannedEmail(models.Model):
    ...
    description = models.CharField(
        ...
        null=False,
        help_text=(
            "A description of when this email should be used, including to which"
            " recipients"
        ),
    )
...
class Email(models.Model):
    ...
    @classmethod
    def FromRawContent(cls, raw_content, **kwargs) -> typing.Self:
    ...
    def ReplyEmail(
        ...
        **kwargs,
        ...
        html = (
            f"<div>{html}</div><div><br/><div>{wrote_str}<br/></div><blockquote"
            ' style="border-left: 1px solid rgb(204,204,204);'
            f' padding-left:1ex">{reply_to_html}</blockquote></div>'
        )
        ...
        if email_message is not None:
            ...
            for reference_ids in email_message.get_all("References", []):
                for reference_id in reference_ids.strip().split():
                    r_id = cls.parseaddr(reference_id)
                    if reference_id:
                        email_kwargs.setdefault("root_reference_id", r_id)
                        email_kwargs.setdefault("reference_ids", []).append(r_id)
    ...
    @classmethod
    def check_is_address_us(cls, address, allow_external: bool = True) -> bool:
    ...
    @classmethod
    def check_is_spam(cls, email_message) -> bool:
        raw_value = email_message.get("X-Spam", "False")
        try:
            value = bool(distutils.util.strtobool(raw_value))
        except ValueError:
            value = False
        return value
    ...
    def get_emails_in_thread_filter(self):
        """
        Makes an intermediary query to check which of the reference emails were
                    ...
                    while last_match != -1:
                        ...
                        if start_line is not None and end_line is not None:
                            ...
                            if start_line:
                                ...
                                try:
                                    tokens: tuple[str]
                                    _, tokens = dateutil.parser.parse(  # type: ignore
    ...
    def claimed_discord_message(self, claimer: typing.Any):
    ...
    def unclaimed_discord_message(self):
        ...
        return (
            f"Email #{self.pk} [{self.subject}] was unclaimed\n"
            f"**Claim email reply:** <{email_answer_url}>\n"
        )
