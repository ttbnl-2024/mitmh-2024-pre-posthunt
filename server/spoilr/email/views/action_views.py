...
def reply_view(request: HqHandlerHttpRequest) -> HttpResponse:
    ...
    if action == AnswerEmailForm.ACTION_NO_REPLY:
        ...
        if confirm is not None and confirm.lower() != "reply":
    ...
    opt_email_in_reply_to: Email | None = (
        Email.objects.select_related("team").filter(pk=email_in_reply_to_pk).first()
    )
    if not opt_email_in_reply_to:
        messages.error(request, "Email being replied to cannot be found!")
        return redirect_with_message(
            request, "spoilr.email:dashboard", "Email reply was not sent."
        )
    email_in_reply_to = opt_email_in_reply_to  # Weird closure non-capture thing
            ...
            def commit_action() -> None:
                if count:
                    dispatch(
                        ...
                        message=(
                            f"Resolved without response email “{email_in_reply_to.id}”"
                        ),
                    )
...
@hq(require_handler=True)
def send_view(request) -> HttpResponse:
    if request.method == "POST":
        ...
        mail.set_content(body_text)
        mail["From"] = f'"{HUNT_TITLE}" <{sender}>'
        mail["To"] = recipient
        ...
        try:
            ...
            log = HqLog.objects.create(
                handler=request.handler,
                event_type="email-send",
                object_id=mail_obj.id,
                message=f"Sent email {mail_obj}",
            )
            log.save()
