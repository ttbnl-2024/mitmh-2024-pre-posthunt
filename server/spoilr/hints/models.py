...
class Hint(models.Model):
    ...
    STATUSES = {
        ...
        ),  # requesters are not expecting an additional reply
    }
    ...
    def populate_response_and_update_requests(self, text_content, status=None):
        ...
        notify_emails: str | set[str] = "none"
        ...
        if not recipients:
            ...
            if reply_to_email:
                ...
                response_email_message["Subject"] = (
                    f"{settings.EMAIL_SUBJECT_PREFIX}Hint answered for"
                    f" {self.puzzle.name}"
                )
                response_email_message["From"] = self.HINT_EMAIL_ADDRESS
    ...
    def __str__(self) -> str:
        ...
        o = f'{self.team.name}, {self.puzzle.name}: "{abbr(self.text_content)}"'
    ...
    @property
    def original_request_id(self) -> int:
    ...
    @property
    def original_request(self) -> "Hint":
        return (
            self if self.root_ancestor_request is None else self.root_ancestor_request
        )
    ...
    @classmethod
    def clean_up_tasks(cls, hints) -> None:
    ...
    @property
    def puzzle_hint_url(self) -> str:
        """url for teams to view their hints for this puzzle."""
        return self.puzzle.tph_puzzle.hints_url

    @property
    def long_status(self) -> str:
        return self.STATUSES.get(self.status, "Invalid status")
    ...
    def created_discord_message(self) -> str:
        ...
        return (
            f"Hint #{self.pk} requested on"
            f" {self.puzzle.tph_puzzle.emoji} {self.puzzle} by"
            f" {self.team}\n**Question:** ```{self.text_content[:1500]}```\n**Puzzle:**"
            f" <{solution_url}> (<{hints_for_this_puzzle}>)\n**Claim and answer hint:**"
            f" <{hint_answer_url}>\n"
        )
    ...
    def unclaimed_discord_message(self) -> str:
        ...
        hint_answer_url = generate_url("hunt", stem, {"hint": self.pk})
        return (
            f"Hint #{self.pk} was unclaimed\n**Claim and answer hint:**"
            f" <{hint_answer_url}>"
        )

    @classmethod
    def responded_discord_message(
        cls, hint_request: "Hint", hint_response: "Hint"
    ) -> str:
        status_name = cls.STATUSES[hint_request.status]
        return (
            f"Hint #{hint_request.pk} resolved by {hint_request.handler}\nHint was"
            " requested on"
            f" {hint_request.puzzle.tph_puzzle.emoji} {hint_request.puzzle} by"
            f" {hint_request.team}\n**Question:**"
            f" ```{hint_request.text_content[:1500]}```\n**Response:**"
            f" {hint_response.text_content}\n**Marked as:** {status_name}\n"
        )
