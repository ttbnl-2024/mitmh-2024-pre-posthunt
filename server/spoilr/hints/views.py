...
class ThreadData(TypedDict):
    hints: list[Hint]
    last_request: Hint | None
    last_response: Hint | None
...
def dashboard_view(request):
    ...
    for hint in hints:
        ...
        form.cleaned_data = {}
        form.initial["hint_request_id"] = hint.id  # type: ignore
        ...
        for h in thread:
            ...
            if h.is_request:
                threadinfo["last_request"] = h
            else:
                threadinfo["last_response"] = h
...
def respond_view(request):
    ...
    with transaction.atomic():
        ...
        # mypy can't realize that the callback is narrowed
        team = hint_request.team
        puzzle = hint_request.puzzle
        ...
        def commit_action():
            dispatch(
                ...
                status=status,
                message=(
                    f"Resolved hint for {team.name} about {puzzle} with status {status}"
                ),
            )
...
def history_view(request, **kwargs):
    ...
    threads_for_same_team: dict[int, ThreadData] = defaultdict(
    ...
    for hint in hints_by_same_team:
        ...
        if hint.is_request:
            threads_for_same_team[hint.original_request_id]["last_request"] = hint
        else:
            threads_for_same_team[hint.original_request_id]["last_response"] = hint
    all_threads = sorted(
    ...
    context["puzzle"] = puzzle
    context["team"] = team
    context["threads_for_same_team"] = all_threads
