...
class TeamRegistrationInfo(models.Model):
    ...
    contact_phone = PhoneNumberField(  # type: ignore
    ...
    bg_playstyle = models.CharField(
        ...
        choices=(
            ...
            (
                "win",
                (
                    "We`re serious about solving. We really want to see the entire Hunt"
                    " and maybe find the coin."
                ),
            ),
        ),
        default="",
    ...
    bg_comm = models.CharField(
        verbose_name=(
            "What communication application will you be using to communicate with your"
            " team members while solving the MIT Mystery Hunt this year?"
        ),
        max_length=200,
        default="",
        blank=True,
    ...
    tb_room = models.CharField(
        ...
        choices=(
            ...
            (
                "maybe",
                (
                    "We would like a room, but we have a backup plan in case Hunt is"
                    " unable to secure a room."
                ),
            ),
    ...
    tb_room_specs = models.CharField(
        verbose_name=(
            "If you are requesting a room, what specifications would you like for your"
            " team base (e.g., specific rooms or room attributes)?"
        ),
        max_length=500,
        default="",
        blank=True,
    ...
    tb_location = models.CharField(
        verbose_name=(
            "If you are not requesting a classroom, where is your team base? We'll be"
            " visiting your team during the weekend. Do we need any special"
            " instructions to access your HQ? If remote, please provide instructions"
            " for how we can contact you virtually (e.g., Zoom link, Discord server,"
            " Google Meet, etc.)."
        ),
        max_length=500,
        default="",
    ...
    other_workshop = models.CharField(
        max_length=20,
        verbose_name=(
            "Would your team like to participate in the How to Hunt workshop prior to"
            " the event?"
        ),
        choices=(("yes", "Yes"), ("no", "No")),
        default="",
    ...
    other = models.CharField(
        verbose_name=(
            "Anything else you`d like to share with us? Comments, questions, puns?"
        ),
        max_length=1500,
        default="",
        blank=True,
    ...
    # Custom for 2024 hunt -- should be in a custom registration model, really, but
    # I couldn't be bothered. It's backend only anyway.
    def constellation_filename(self, filename: str):
        return f"constellations/{self.team_name}-{self.id}-{filename}"

    constellation_pic = models.ImageField(
        upload_to=constellation_filename,  # type: ignore
        max_length=300,
        null=True,
    )
...
class IndividualRegistrationInfo(models.Model):
    ...
    bg_mh_history = models.CharField(
        verbose_name=(
            "Have you participated in the MIT Mystery Hunt before? If so, tell us about"
            " your Hunt history!"
        ),
        max_length=500,
        default="",
    ...
    bg_other_history = models.CharField(
        verbose_name=(
            "Have you played in other puzzle-type events before? If so, feel free to"
    ...
    bg_playstyle = models.CharField(
        ...
        choices=(
            ...
            (
                "win",
                (
                    "I'm a puzzle machine, and I'd love to be on a team that wants to"
                    " find the coin. I really want to see the whole hunt."
                ),
            ),
        ),
        default="",
    ...
    bg_other_prefs = models.CharField(
        verbose_name=(
            "Do you have any other preferences about what team you want to join?"
        ),
        max_length=500,
        default="",
        blank=True,
    ...
    def __str__(self):
        ret = self.contact_first_name
        if self.contact_last_name:
            ret += " " + self.contact_last_name
        if self.contact_pronouns:
            ret += f" ({self.contact_pronouns})"
        return ret
