...
from spoilr.core import models
...
def send_email_for_update(update):
    ...
    for _team, emails in team_and_emails:
        if not emails:
            continue
        send_email(
            update.subject,
...
def updates_view(request):
    if request.method == "POST":
        updates_to_publish = HQUpdate.objects.filter(
    ...
    hqupdates = HQUpdate.objects.order_by("-creation_time")
