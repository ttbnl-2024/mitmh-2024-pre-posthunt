...
class InjectedTeamHttpRequest(HttpRequest):
    team: Team


def inject_puzzle(*, allow_admin=True, error_if_inaccessible=True) -> ViewDecorator:
    """
...
def inject_interaction(
    *, allow_admin=True, error_if_inaccessible=True
) -> ViewDecorator:
    """
    ...
    object into the request as a `team_interaction` attribute if the current team
    ...
    request is served but `team_interaction` will be `None`.
    ...
    def decorator(view_func: ViewFunc) -> ViewFunc:
        ...
        def wrapped(request, *args, **kwargs):
            ...
            request.team_interaction = None
            if team:
                request.team_interaction = TeamInteraction.objects.filter(
                    team=team, interaction=request.interaction
                ).first()

            if not request.team_interaction:
                if error_if_inaccessible and not (
                    allow_admin and request.user.is_staff
                ):
                    logger.warning(
...
def require_launch(site_ref, allow_admin=True, redirect_target=None) -> ViewDecorator:
    """
...
def require_safe_referrer(view_func: ViewFunc) -> ViewFunc:
...
def get_inaccessible_interaction_response(interaction_slug):
    return HttpResponseNotFound("Cannot find interaction " + escape(interaction_slug))
